...
@app.post("/get_axis")
def generate_pdf(axis_points: Annotated[str, Form()],input_path:Annotated[str,Form()],production_id:Annotated[int,Form()],db:Session=Depends(get_db)):
    x_axis_y_axis = axis_points
    input_path = input_path
    output_path = input_path[:-4]+'_output.pdf'
...
def get_axis_audit(axis_points: Annotated[str, Form()],input_path:Annotated[str,Form()],production_id:Annotated[int,Form()],db:Session=Depends(get_db)):
    x_axis_y_axis = axis_points
    input_path = input_path
    output_path = input_path[:-4]+'_audit.pdf'
    x_axis_y_axis = x_axis_y_axis.replace("'", '"')
    marker_positions =  json.loads(x_axis_y_axis)
    html_width = 0
    html_height = 0
